...
import com.zsy.shunai.model.dto.app.*;
...
public class AppController {
    ...
    public BaseResponse<Long> addApp(@RequestBody AppAddRequest appAddRequest, HttpServletRequest request) {
        ...
        //  在此处将实体类和 DTO 进行转换
        // app中没有需要进行JSON转换的内容，所以可以直接BeanUtils.copyProperties()复制一下
        App app = new App();
    ...
    public BaseResponse<Boolean> reviewApp(@RequestBody AppReviewRequest appReviewRequest, HttpServletRequest request) {
        if (appReviewRequest == null || appReviewRequest.getId() <= 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }

        long id = appReviewRequest.getId();
        Integer reviewStatus = appReviewRequest.getReviewStatus();
        ...
        ThrowUtils.throwIf(Objects.equals(reviewStatus, oldApp.getReviewStatus()), ErrorCode.PARAMS_ERROR, "请勿重复审核");

        if (reviewMessage != null && reviewMessage.length() > 80) {
    ...
    public BaseResponse<Page<AppVO>> listAppVOByPage(@RequestBody AppQueryRequest appQueryRequest,
        ...
        // 只能看到已过审的应用
        // 查询请求里指定审核状态==PASS，那么只有审核状态==PASS的应用才会被查到
